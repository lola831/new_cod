...
const Trip = () => {
  ...
  return (
    <div className="px-4 sm:px-10 py-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 bg-white">
      {dataTrips.map((trip) => (
        <div key={trip.id} className={`flex flex-col border-2 rounded-xl overflow-hidden group md:hover:bg-white md:hover:shadow-xl md:transition-all md:duration-300 md:ease-in-out md:hover:-translate-y-1 md:hover:scale-105`}>
          ...
          <div className="flex flex-col flex-grow p-4 antialiased leading-snug">
            ...
            <p className="text-gray-700 flex-grow text-sm md:text-base lg:text-base">
              Price: ${trip.price} <br/>
              Max Capacity: {trip.capacity}
            </p>

            <p className="text-sm md:text-base lg:text-base font-normal antialiased leading-relaxed text-gray-500 overflow-ellipsis overflow-hidden flex-grow mt-2">
              {trip.description}
            </p>
